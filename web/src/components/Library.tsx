...
export function Library({ onClose, onChange }: { onClose: () => void; onChange: (t: Take) => void }) {
  ...
  return (
    <aside className="drawer" aria-label="Library">
      ...
      <p className="hint">
        Sounds you saved with ☆, kept until you remove them. To download several at once, add them to the
              ...
              {list.map((t) => (
                <li key={t.id} className="drawer__item">
                  ...
                  <button type="button" className={`icon-link${basket.has(t.id) ? ' is-on' : ''}`} onClick={() => basket.toggle(t.id)}
                    title={basket.has(t.id) ? 'In the basket (click to remove)' : 'Add to the basket'}>{basket.has(t.id) ? '✓' : '+'}</button>
